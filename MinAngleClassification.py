...
# There are over 200 images for every class, so we will go up to 100 images, but once we pass 10 we will start doing pca
# and/or lda
NUM_ITERS = 10
NUM_TRAIN_ANGLES = 100
...
for i in range(NUM_ITERS):
    ...
    for dir in glob.glob('fruits-360/Training/*'):
        # Choose random image from each directory
        paths = random.sample(glob.glob(dir + "/*.jpg"), NUM_TRAIN_ANGLES)
        for name in paths:
            img = cv2.imread(name)
            # Invert the colors from BGR -> RGB
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    ...
    # X_lda = lda.fit_transform(images_scaled, classification)
    ...
    # X_pca = pca.fit_transform(images_scaled, classification)
